    ...
    try:
        ...
        if not message.reply_to_message.empty:
            msg = await message.reply_to_message.reply_text(
                "Downloading image to @imageditortrbot", quote=True
            )
            a = await client.download_media(
                message=message.reply_to_message, file_name=download_location
            )
            await msg.edit("Processing Image...at @imageditortrbot")
        ...
        if "USER_IS_BLOCKED" in str(e):
            ...
            try:
                await message.reply_to_message.reply_text(
                    "Something went wrong!Use Original Bot @imageditortrbot", quote=True
                )
            except Exception:
                return
...
async def circle_without_bg(client, message):
    try:
        ...
        if not message.reply_to_message.empty:
            ...
            await msg.edit("Processing Image...on @imageditortrbot")
            img = Image.open(a).convert("RGB")
            npImage = np.array(img)
            h, w = img.size
            alpha = Image.new("L", img.size, 0)

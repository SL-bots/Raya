...
from Raya.modules.utils.disable import disableable_dec
...
async def get_start_func(message, strings, edit=False):
    ...
    buttons.add(
        ...
        InlineKeyboardButton(
            strings["btn_source"], url="https://github.com/SLBotsOfficial/"
        ),
    )
    buttons.add(
    ...
    buttons.add(
        InlineKeyboardButton(
            "👸🏼 Add Raya to your group",
            url=f"https://telegram.me/TheRayaRobot?startgroup=true",
        )
    )
...
async def help_cmd_g(message, strings):
    ...
    button = InlineKeyboardMarkup().add(
        InlineKeyboardButton(text=text, url="https://t.me/TheRayaRobot?start")
    )
    await message.reply(strings["help_header"], reply_markup=button)

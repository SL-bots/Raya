...
from Raya.services.mongo import mongodb as db
...
async def _(event):
    ...
    await tbot.edit_message(
        chatid,
        msgid,
        "Raya found some torrents for you. Take a look 👇",
        buttons=[
            [
                Button.inline(
                    "📤 Get Torrents from SLBotsOfficial's API",
...
async def paginate_prevtorrent(event):
    ...
    try:
        ...
        await event.reply("Sorry, Raya Cant found any torrents for that word")
        print(e)
        return
    vector = len(results)
    if num < 0:
...
Special Credits to Tharuk for api and also for TG/Torrent project
"""

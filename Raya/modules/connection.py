...
from Raya.services.redis import redis
...
async def connect_to_chat_direct(message, strings):
    ...
    try:
        ...
        redis.set("Raya_connected_start_state:" + str(user_id), 1)
...
async def connected_start_state(message, strings, chat):
    key = "Raya_connected_start_state:" + str(message.from_user.id)
    if redis.get(key):
        await message.reply(
...
__help__ = """
Sometimes you need change something in your chat, like notes, but you don't want to spam in it, try connections, this allow you change chat settings and manage chat's content in personal message with Raya.
